...
- [people.v2.csv](people.v2.csv) - JMeter Тест-план для тестирования fail over при вставке в таблицу пользователей
...
1. Убедиться, что приложение запущено и доступно (http://localhost:8001/health)
...
Нагрузка перешла на реплики примерно поровну. На местере нагрузки нет.
...
![Потребление реплиой 2 ресурсов](images/stage-2-replicas-handle-read-requests-replica_2-usage.png)
...
1. Настроить параметры Thread Group (количество пользователей, время выполнения)
1. Запустить тест
1. На середине от ожидаемой продолжительности НТ произвести отключение мастера, промоут реплики, настройки ее репликации в качестве нового мастера, изменения коннекта к новому мастеру в конфигурации второй реплики:
...
1. Проанализировать результаты через Listeners. В этот раз нас будет интересовать только аргегированный отчет.
1. Вернуть оригинальную конфигурацию с кворумной репликацией и изначальным мастером
    ...
    # Поднять старый мастер и сделать промоут
...
Профиль состоит из единственного сценария. Сценарий состоит из единственной операции регистрации пользователи в сервисе `/user/register`.
...
Стало на реплике новом мастере
...
*Рис 9. Статистика успешных запросов в *
